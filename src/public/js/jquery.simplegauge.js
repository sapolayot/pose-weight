...
(function ($) {
  ...
  SimpleGauge.prototype = {
    ...
    draw: function () {
      ...
      this.$error = this.$element.find(".simpleGauge_error");
      this.setContainer();
      this.setTitle();
      this.setError();
    ...
    setContainer: function () {
      ...
      if (this.options.container) {
        if (this.options.container.style) {
          ...
          let css = this._styleToCss(
            style,
            $.fn.simpleGauge.defaults.container.style,
          );
          this.$gauge.css(css);
        }
    ...
    setTitle: function () {
      if (this.options.title && this.options.title.text) {
        ...
        let css = this._styleToCss(
          style,
          $.fn.simpleGauge.defaults.title.style,
        );
        if (css.top) {
    ...
    setError: function () {
      if (this.options.error && this.options.error.text) {
        this.$error.html("<span>" + this.options.error.text + "</span>");
        let style = this.options.error.style;
        let css = this._styleToCss(
          style,
          $.fn.simpleGauge.defaults.error.style,
        );
        if (css.top) {
          this.$error.css({ top: css.top });
          delete css.top;
        }
        this.$error.find("> span").css(css);
      }
    },
    ...
    getAngleFromValue: function (value) {
      return (
        ((value - this.options.min) / (this.options.max - this.options.min)) *
          (this.options.analog.maxAngle - this.options.analog.minAngle) +
        this.options.analog.minAngle
      );
    },
    ...
    /*
        getPointFromAngleUNUSED: function (w, h, angle) {
            ...
            let tan = Math.tan(a);
            let div = Math.pow(h * h + w * w * tan * tan, 0.5); // âˆšh2+w2(tanðœƒ)2
            let x = w * h / div;
            let y = w * h * tan / div;
...
*/
    ...
    createAnalogBars: function () {
      ...
      this.options.bars.colors.forEach((colorArr, idx) => {
        ...
        let endVal =
          idx + 1 > maxIdx
            ? this.options.max
            : this.options.bars.colors[idx + 1][0];
    ...
    createAnalogLabels: function () {
      ...
      let step =
        (this.options.max - this.options.min) / this.options.labels.count;
      ...
      for (let val = this.options.min; val <= this.options.max; val += step) {
        ...
        let labelVal = val;
        let html = labelTemplate.replace(
          /\{value(?:\.(\d+))?\}/g,
          function (m, c1) {
            if (c1) {
    ...
    createAnalogTicks: function () {
      ...
      if (this.options.ticks.count) {
        let step =
          (this.options.max - this.options.min) / this.options.ticks.count;
        let width1 =
          (this.options.gaugeWidth * this.options.ticks.scale1) / 100;
        let width2 =
          (this.options.gaugeWidth * this.options.ticks.scale2) / 100;
        let width = (width2 + width1) / 2;
        let height1 =
          (this.options.gaugeHeight * this.options.ticks.scale1) / 100;
        let height2 =
          (this.options.gaugeHeight * this.options.ticks.scale2) / 100;
        let height = (height2 + height1) / 2;
      ...
      if (this.options.ticks.count && this.options.subTicks.count) {
        ...
        let width1 =
          (this.options.gaugeWidth * this.options.subTicks.scale1) / 100;
        let width2 =
          (this.options.gaugeWidth * this.options.subTicks.scale2) / 100;
        let width = (width2 + width1) / 2;
        let height1 =
          (this.options.gaugeHeight * this.options.subTicks.scale1) / 100;
        let height2 =
          (this.options.gaugeHeight * this.options.subTicks.scale2) / 100;
        let height = (height2 + height1) / 2;
        let tickHeight = Math.abs(height2 - height1);
        for (let val = this.options.min; val <= this.options.max; val += step) {
          let angle = this.getAngleFromValue(val);
    ...
    createAnalogPointer: function () {
      let css = this._styleToCss(
        this.options.pointer.style,
        $.fn.simpleGauge.defaults.pointer.style,
      );
      this.$pointers.css({
        marginLeft: this.options.gaugeWidth / 2,
        marginTop: this.options.gaugeHeight / 2,
    ...
    setDigital: function () {
      if (this.options.digital && this.options.digital.text) {
        ...
        let css = this._styleToCss(
          style,
          $.fn.simpleGauge.defaults.digital.style,
        );
        if (css.top) {
  ...
  $.fn.simpleGauge = function (options, val) {
    ...
    this.each(function () {
      ...
      if (isInit) {
        ...
        $this.data(
          "plugin-simplegauge",
          (data = new SimpleGauge($this, options)),
        );
  ...
  $.fn.simpleGauge.defaults = {
    template: [
      ...
      '<div class="simpleGauge_error"></div>',
