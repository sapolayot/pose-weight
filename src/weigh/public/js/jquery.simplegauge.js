...
(function($) {
    ...
    */
    ...
    createAnalogBars: function() {
      ...
      this.options.bars.colors.forEach((colorArr, idx) => {
        ...
        let d = "M " + startCoord + " A " + width / 2 + " " + height / 2 + " 0 " + (Math.abs(endAngle - startAngle) > 180 ? 1 : 0) + " 1 " + endCoord;
    ...
    createAnalogTicks: function() {
      ...
      if (this.options.ticks.count && this.options.subTicks.count) {
        let step = (this.options.max - this.options.min) / (this.options.ticks.count * this.options.subTicks.count);
  ...
  $.fn.simpleGauge = function(options, val) {
    ...
    if (isInit && options.debug != void 0) {
      debug = options.debug;
    }
    let result = null;
    this.each(function() {
